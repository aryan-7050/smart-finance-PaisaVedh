...
export class RecurringService {
  ...
  async detectRecurringExpenses(userId: string): Promise<RecurringPattern[]> {
    ...
    for (const group of transactions) {
      ...
      if (confidence > 0.7) { // 70% confidence threshold
        patterns.push({
          ...
          frequency,
          confidence,
          transactionIds: group.transactions.map((t: any) => t._id.toString())
        });
      }
    }
  ...
  private detectFrequency(dates: Date[]): 'daily' | 'weekly' | 'monthly' | 'yearly' {
  ...
  private calculateConfidence(dates: Date[], frequency: string): number {
    if (dates.length < 2) return 0;
    
    let expectedIntervals: number[] = [];
    switch(frequency) {
      case 'daily': expectedIntervals = [1]; break;
      case 'weekly': expectedIntervals = [7, 14, 21, 28]; break;
      case 'monthly': expectedIntervals = [28, 29, 30, 31, 35]; break;
      case 'yearly': expectedIntervals = [365]; break;
    }
    ...
    for (let i = 1; i < sortedDates.length; i++) {
      const diffDays = (sortedDates[i].getTime() - sortedDates[i-1].getTime()) / (1000 * 3600 * 24);
      const closestExpected = expectedIntervals.reduce((prev, curr) => 
  ...
  private async saveRecurringPatterns(userId: string, patterns: RecurringPattern[]): Promise<void> {
    for (const pattern of patterns) {
      // Check if pattern already exists
      const exists = await RecurringExpense.findOne({
        userId: new mongoose.Types.ObjectId(userId),
      ...
      if (!exists) {
        ...
        const lastTransaction = await Transaction.findById(pattern.transactionIds[pattern.transactionIds.length - 1]);
