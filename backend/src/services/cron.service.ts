...
export class CronService {
  ...
  private async generateMonthlyReports(): Promise<void> {
    ...
    for (const user of users) {
      try {
        ...
        const topCategories = transactions.sort((a, b) => b.total - a.total).slice(0, 5);
  ...
  private async checkBudgetAlerts(): Promise<void> {
    ...
    const monthProgress = (now.getDate() - 1) / (new Date(currentYear, currentMonth + 1, 0).getDate());
  ...
  private async cleanupOldData(): Promise<void> {
    ...
    // Archive old transactions (optional: move to archive collection)
    const result = await Transaction.deleteMany({
      date: { $lt: oneYearAgo },
      isRecurring: false // Keep recurring transactions
    });
  ...
  private async generateWeeklyInsights(): Promise<void> {
    // Implementation for weekly AI insights
    // Similar to monthly but weekly
